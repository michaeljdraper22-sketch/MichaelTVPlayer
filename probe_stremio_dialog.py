...
def new_tmp(prefix):
    """Tracked mkdtemp: every dir the probe creates is rmtree'd in the
    finally at the bottom — no mtp_probe_sdlg_ litter left in TEMP."""
...
try:
    ...
    PATCH_STATE["status"] = {"found": False, "patched": False,
                             "backup": False, "titled": False}
    ...
    check("server.js missing: neutral status, button disabled",
          ...
          dlg.patch_lbl.text())
    PATCH_STATE["status"] = {"found": True, "patched": False,
                             "backup": True, "titled": False}
    dlg._refresh_patch_status()
    ...
    tmp = Path(new_tmp("mtp_probe_sdlg_")) / "settings.json"
    ...
    stmp = Path(new_tmp("mtp_probe_sdlg_")) / "settings.json"
    ...
    if not stremio_profile._leveldb_dirs():
        ...
        def _nonsecret(urls):
            """Redacted extra for check(): counts + hostnames only —
            the full URLs carry TorBox/Premiumize API keys and must
            never reach stdout, even on a failed check."""
            hosts = sorted({(urlparse(str(u)).hostname or "?")
                            for u in (urls or [])})
            return "n=%d hosts=%s" % (len(urls or []),
                                      ",".join(hosts) or "-")

        check("live: URLs survive the config setter verbatim (textarea "
              "format parity)", live_cfg.stremio_addons == ordered_urls,
              "%s vs %s" % (_nonsecret(live_cfg.stremio_addons),
                            _nonsecret(ordered_urls)))
    ...
    for d in _tmpdirs:               # no mtp_probe_sdlg_ litter in TEMP,
        shutil.rmtree(d, ignore_errors=True)   # however the probe ends
